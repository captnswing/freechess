...
class GameManager(models.Manager):
    def won_games(self):
        return super(GameManager, self).get_queryset().filter((Q(self_white=True) & Q(result="1-0")) |
                                                              (Q(self_white=False) & Q(result="0-1")))

    def lost_games(self):
        return super(GameManager, self).get_queryset().filter((Q(self_white=True) & Q(result="0-1")) |
                                                              (Q(self_white=False) & Q(result="1-0")))
...
class ChessGame(models.Model):
    ...
    def __repr__(self):
        return "#%s  %s (%s) - %s (%s): %s" % (
            self.game_nr, self.self_white, self.self_elo, self.opponent_name, self.opponent_elo, self.result)
    ...
    class Admin:
        list_display = (
            'game_nr', 'date', 'self_white', 'opponent_name', 'self_elo', 'opponent_elo', 'timecontrol', 'result',
            'comment')
        ordering = ['game_nr']
